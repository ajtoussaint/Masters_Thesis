...
input_file = os.path.join(path, "data/2012-2013-data-with-predictions-4-final.csv")
...
def wrangle(overwrite=False):
    if(overwrite or not os.path.exists(q_matrix_output_file) or not os.path.exists(response_matrix_output_file)):
        print("Wrangling ASSIST12 data...")
        #read in the data
        df = pd.read_csv(input_file)
        df = df[['problem_id', 'user_id', 'correct', 'skill_id']]
        ...
        #ensure each user has completed at least 15 problems, and each problem has 15 responses
        ...
        print("Using pre-existing wrangled data for ASSIST12")
...
def createQ(df):
    ...
    for skill in tqdm(skill_list, "parsing skills to build Q matrix: "):
      ...
      for problem in problems:
        skills = df.loc[df["item_id"] == problem]["skill_id"].tolist()[0]
        if(skill == skills):
